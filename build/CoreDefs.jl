...
function collar(tile, patch)
    ...
    result = [tile]
    for t in patch
        if t != tile && any(v -> v in vs, vertices(t))
            push!(result, t)
        end
    end
    return result
...
function collars(S, n)
    ...
    result = []
    for ptile in ptiles
        stile = substitute(S, [(e, ptile)], n)
        #println(stile)
        for tile in stile
            if !any(v -> in_border(v//S.λ^n, ptile), vertices(tile)) && !any(t -> peq(t,center_patch(collar(tile, stile))), result)
                push!(result, center_patch(collar(tile, stile)))
            end
        end
    end
    return result
